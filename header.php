				...
				<div class="row">	
						...
						<?php 
							...
							foreach($Langs as $Lang) {
								...
								if(strpos($Request, 'register') !== false) {
									...
									$Url = SITE_URL . 
												 $Lang['slug'] . '/register' . 
												 ($_GET['user_redirect'] ? '/?user_redirect=' . $_GET['user_redirect'] : '');

								} elseif(strpos($Request, 'profile-settings') !== false) {

									$Url = SITE_URL . $Lang['slug'] . '/profile-settings';
									
								} elseif(strpos($Request, 'profile-links') !== false) { 

									$Url = SITE_URL . $Lang['slug'] . '/profile-links';
									
								} elseif(strpos($Request, 'profile') !== false) {

									$Url = SITE_URL . $Lang['slug'] . '/profile/' . $User->user_login;
									
								} elseif(strpos($Request, 'author') !== false) {

									$Url = SITE_URL . ($Lang['slug'] == 'es' ? 'es/' : '') . 'author/' . $User->user_login;
					...
					<div class="header-options right clearfix">					
						...
						<?php if(is_user_logged_in()) { ?>
							...
							<div class="button-wrap left UserRole">
								<a href="<?php echo ThemexUser::$data['user']['profile_url']; ?>" 
									 class="element-button">
									<span class="button-icon register"></span>
									<span><?php _e('My Profile','academy'); ?></span>									
									<!-- ?php 
										$User = wp_get_current_user();
										$Role = $User->roles[0]; 
										if($Role == 'subscriber') {

											echo '(<span class="IsSoci">' . __('Subscriber','academy') . '</span>)';

										} elseif ($Role == 'customer') {

											echo '(<span class="IsSoci">' . __('Customer','academy') . '</span>)';
										}
									? -->									
								</a>						
							</div>							
