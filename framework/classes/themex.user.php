...
class ThemexUser {
	...
	public static function registerUser($data) {
      ...
      empty($data['user_password_repeat']
        ...
        $data['user_login']) ||
        is_email($data['user_login']) ||
        preg_match('/\s/', $data['user_login'])
      ) {
	...
	public static function resetPassword($data) {
		...
		if(email_exists(sanitize_email($data['user_email']))) {
			...
			if(!$allow || is_wp_error($allow)) {
				...
				// $message=ThemexCore::getOption('email_password', 'Hi, %username%! <a href="%link%">Click this link to reset your password %link%</a>'); 
				$message=ThemexCore::getOption(
					'email_password', 
					sprintf(
						__('Click this link to reset your password', 'academy'),
						$login, 
						get_bloginfo('name')
					) . 
					' <a href="%link%">%link%</a>'
    ...
    // is soci
		...
		$issoci=themex_value($data, 'confirm_subscriber');
