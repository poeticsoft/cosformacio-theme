		...
		if($user!=0 || (self::$data['status']=='premium' && self::$data['product']!=0)) {
			...
			if(!self::isMember($user)) {
				...
				if(
					...
					(self::$data['status']=='premium' && self::$data['product']==0)
				) {
					...
					self::$data['status']=='premium'
				) {

					/* NOT REGISTERED USERS SHOULD GO TO REGISTE FORM BEFORE TAKE A COURSE
					https://themex.co/support/topic/automatically-redirect-guest-to-registration-page-during-check-out
					...
					if(is_user_logged_in()) {
						...
						wp_redirect('/' . pll_current_language() . '/register?user_redirect=' . $ID);
						exit();
					}
				}
			} else if(!$private) {
