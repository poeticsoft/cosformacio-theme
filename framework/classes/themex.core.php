...
class ThemexCore {
	...
	public static function enqueueStyle($args) {
		if(isset($args['uri'])) {
			wp_enqueue_style($args['name'], $args['uri'], array(), '00.01');
		} else {
	...
	public static function uploadImage($file) {
		...
		if(!empty($file['name'])) {
			...
			$filename=wp_unique_filename($uploads['path'], 'image-'.$fileid.'.'.$filetype['ext']);
	...
	public static function rewriteRule($rule) {
		...
		if(isset($rule['replace']) && $rule['replace']) {
			...
			if($rule['rule']=='author_base') {

				$wp_rewrite->author_base=$rule['rewrite'];
	...
	public static function getURL($name, $value=1) {
		...
		if(isset($rule['name'])) {
			...
			$slug = strpos($rule['name'], 'register') !== false ? pll_current_language() . '/' . $rule['name'] : $rule['name'];
		}
