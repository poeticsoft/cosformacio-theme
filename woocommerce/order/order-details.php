					...
					<?php
						...
						if($_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted()){
							...
							foreach($download_files as $download_id => $file ){
								...
								$links[] = '<small><a href="' . esc_url($file['download_url'] ) . '">' . sprintf(__('Download file%s', 'academy' ),(count($download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html($file['name'] ) . '</a></small>';
							}
	...
	<tbody>
		<tr>
			...
			<td>
				...
				<strong><?php _e('Email:', 'academy'); ?></strong>&nbsp;<?php echo $order->get_billing_email(); ?><br />
				<?php } ?>
				<?php if($order->get_billing_phone()!='') { ?>
				<strong><?php _e('Phone:', 'academy'); ?></strong>&nbsp;<?php echo $order->get_billing_phone(); ?>
				<?php } ?>
